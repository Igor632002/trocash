...
import { getTopVisibleListings } from "@/lib/uiHelpers";
import { buildOfferPayload, validateOfferForm } from "@/lib/domain/offers";
...
export function useOffers({ user, copy, category, onPublished } = {}) {
  ...
  useEffect(() => {
    ...
    async function loadTop() {
      try {
        const top = await fetchTopOffers(16);
        if (!mounted) return;
        setVisibleListings(prev => {
          if (prev && prev.length) return prev;
          setIsTopList(true);
          return getTopVisibleListings({ offers: top, limit: 16 });
        });
      } catch (e) {
        console.error("fetchTopOffers error", e);
      }
    }
    loadTop();
  ...
  async function proposeExchange(offerId, targetId) {
    if (!user?.id) {
      setNotice("Please sign in to request an exchange.");
      return;
    }
    try {
      await createExchangeRequest({ offerId, proposerId: user.id, targetId });
