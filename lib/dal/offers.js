...
import { uploadPhotosAndGetUrls } from "@/lib/dal/storage";

export async function fetchActiveOffers() {
  const { data, error } = await supabase
    .from("offers")
    .select("*, profiles(display_name, area,email), category(id, name)")
    .eq("status", "active")
...
export async function insertOffer(form, userOrId) {
  ...
  const payload = {
    owner_id: userId,
    ...form,
    // безпечне приведення: якщо поле не передано — за замовчуванням false
    is_top: form?.is_top ? true : false,
    status: "active",
  };
  ...
  // Resolve location_id from the area name so offers link to the location table
  try {
    if (!payload.location_id && payload.area) {
      const { data: locRows, error: locErr } = await supabase
        .from("location")
        .select("id")
        .eq("name", payload.area)
        .limit(1);
      if (locErr) {
        console.warn("lookup location failed", locErr);
      } else if (locRows && locRows.length) {
        payload.location_id = locRows[0].id;
      }
    }
  } catch (err) {
...
export async function createExchangeRequest(offerOrPayload, proposerId) {
  const offerId = typeof offerOrPayload === "object"
    ? (offerOrPayload?.offerId || offerOrPayload?.offer_id)
    : offerOrPayload;
  const proposer = typeof offerOrPayload === "object"
    ? (offerOrPayload?.proposerId || offerOrPayload?.proposer_id || offerOrPayload?.from || offerOrPayload?.userId)
    : proposerId;

  let resolvedProposer = proposer;
  if (!resolvedProposer) {
    const { data: sessionData } = await supabase.auth.getSession();
    resolvedProposer = sessionData?.session?.user?.id;
  }

  if (!offerId) throw new Error("Missing offer id for createExchangeRequest");
  if (!resolvedProposer) throw new Error("Missing proposer id for createExchangeRequest");

  const { error } = await supabase.from("exchange_requests").insert([
    {
      offer_id: offerId,
      proposer_id: resolvedProposer,
...
export async function fetchOfferById(id) {
  ...
  const { data, error } = await supabase
    .from("offers")
    .select("*, profiles(display_name, area,email), category(id, name), location(id, name)")
    .eq("id", id)
    .limit(1)
    .maybeSingle();

  if (error) {
    // Log full response for debugging (server-side)
    try {
      console.error("fetchOfferById supabase response", { data, error });
    } catch (e) {
      console.error("fetchOfferById supabase response logging failed", e);
    }
    throw error;
  }
  return data || null;
...
export async function fetchTopOffers(limit = 16) {
  const { data, error } = await supabase
    .from("offers")
    .select("*, profiles(display_name, area,email), category(id, name), location(id, name)")
    .eq("status", "active")
